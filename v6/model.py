...
#Read csv files
df_train = pd.read_csv("./training_data/merged_train_i1_f.csv", sep=';')
...
#Create loss function - to overcome class inbalance (v2)

label_sums = df_train[label_columns].sum(axis=0)
...
p_class_weights = torch.tensor(p_class_weights.values, dtype=torch.float)
p_class_weights = p_class_weights.to(device)

loss_function = BCEWithLogitsLoss(pos_weight=p_class_weights)
...
#Create custom compute_loss func in trainer
